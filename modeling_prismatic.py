...
class PrismaticVisionBackbone(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        if not self.use_fused_vision_backbone:
            print("Using single featurizer for processing.")
            featurized_output = self.featurizer(pixel_values)
            print(f"Featurized output shape (single): {featurized_output.shape}")
        ...
        # Concatenate feature maps along the channel dimension (dim=2)
...
class PrismaticProjector(nn.Module):
    ...
    def forward(self, img_patches: torch.Tensor) -> torch.Tensor:
        ...
        if not self.use_fused_vision_backbone:
            ...
            print(f"Shape after fc1: {projected_features.shape}")

            # Activation function 1 (act_fn1)
            projected_features = self.act_fn1(projected_features)
            print(f"Shape after act_fn1: {projected_features.shape}")

            # Second linear layer (fc2)
            projected_features = self.fc2(projected_features)
            print(f"Shape after fc2: {projected_features.shape}")
            ...
            # Activation function 2 (act_fn2)
            projected_features = self.act_fn2(projected_features)
            print(f"Shape after act_fn2: {projected_features.shape}")

            # Third linear layer (fc3)
            projected_features = self.fc3(projected_features)
            print(f"Shape after fc3: {projected_features.shape}")
